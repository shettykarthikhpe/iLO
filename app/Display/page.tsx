...
const Display: React.FC<PageProps> = ({ ip, username, password }) => {
 ...
 if (res.data.success) {
 setIpList(prev => [...prev, { ip: newIp, username: newUser, password: newPass }]);
 setNewIp(""); setNewUser(""); setNewPass("");
 setShowDialog(false);
 }
 } catch (err) {
 console.error(err);
 ...
 useEffect(() => { tokenGetter(); }, []);
 useEffect(() => { if (token) getIp(); }, [token]);
 useEffect(() => {
 ...
 <div style={styles.sidebar}>
  ...
  <Button onClick = {() => window.location.reload()}><Avatar src="/iLOlogo.png" sx={{width: 60, height: 60}} />  <h3 style={styles.sidebarTitle}>IP List</h3></Button>
